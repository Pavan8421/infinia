#!/usr/bin/env python3
"""Phase 4 step 1: eval Arabic XTTS clips — WER + speaker similarity + latency/RTF."""
...
def main() -> None:
    ...
    wav_dir = args.wav_dir or (ROOT / "outputs" / "arabic" / "xtts")
    ...
    for item in sentences:
        ...
        if not wav_path.is_file():
            print(f"MISSING — run scripts/run_arabic.py first")
            continue
    ...
    out_json = ROOT / "results" / "arabic_xtts_eval.json"
    ...
    payload = {
        ...
        "cloning": "cross-lingual from reference wav",
        "clips": rows,
        "averages": {
            "wer_pct": avg_wer,
            "cosine": avg_cos,
