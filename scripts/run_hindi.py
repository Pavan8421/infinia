#!/usr/bin/env python3
"""Generate Hindi XTTS-v2 clips for the locked test sentences (Phase 4 Hindi step 2)."""
...
from pipelines.hindi import default_hindi_pipeline
...
def main() -> None:
    ...
    print(f"language: hi · cross-lingual clone from reference")
    print(f"sentences: {len(items)}")

    pipe = default_hindi_pipeline(ROOT, device=args.device)
    pipe.load()
