#!/usr/bin/env python3
"""Eval Arabic MMS-TTS clips — WER + speaker similarity + latency/RTF."""
...
def main() -> None:
    ...
    wav_dir = args.wav_dir or (ROOT / "outputs" / "arabic" / "mms")
    ...
    print(f"asr language: ar")
    print("note: MMS is fixed-speaker — cosine vs ref WAV is informational")
    ...
    for item in sentences:
        ...
        if not wav_path.is_file():
            print("MISSING — run scripts/run_arabic_mms.py first")
            continue

        t0 = time.perf_counter()
        wer_out = round_trip_wer(wav_path, text, language="ar", device=args.device)
        print(
    ...
    out_json = ROOT / "results" / "arabic_mms_eval.json"
    ...
    payload = {
        ...
        "model": "mms_tts",
        "model_name": cfg.get("models", {}).get("mms_tts", {}).get(
        ...
        "cloning": False,
        "cloning_note": (
            "Fixed MMS Arabic speaker; not WAV cloning. "
