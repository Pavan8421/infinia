#!/usr/bin/env python3
"""Generate Arabic MMS-TTS clips for the locked test sentences."""
...
from pipelines.arabic_mms import default_arabic_mms_pipeline
...
def main() -> None:
    ...
    print(f"model:    {mms.get('model_name', 'facebook/mms-tts-ara')}")
    print(f"language: ar (MSA) · fixed MMS speaker (NOT wav cloning)")
    print(f"sentences: {len(items)}")

    pipe = default_arabic_mms_pipeline(ROOT, device=args.device)
    pipe.load()
